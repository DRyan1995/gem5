...
do
	...
	build/ALPHA/gem5.opt configs/example/fs.py --cpu-type=DerivO3CPU --script=run_scripts/${j}_1c_simsmall.rcS \
			     --num-cpus=1 --caches --l2cache --victim_cache --l2_size=512kB --l1d_size=32kB --l1i_size=32kB --l1d_assoc=${i} \
	...
	cp m5out/stats.txt ../results/${j}_${i}_8result.txt
    cp m5out/config.ini ../results/${j}_${i}_8config.ini
	echo "****************finish ${j} ${i} "
	echo " "
    done
done
